...
graphs_dir = os.path.join(rootdir, 'data', 'graphs')

def graphData(Kp, Ki, Kd, setpointA, setpointB, ffwda, ffwdb, start_time, filename, df):
    df = pd.DataFrame(df, columns = ['timestamp', 'inputA', 'outputA', 'a_adjust', 'errorA',  'inputB', 'outputB', 'b_adjust', 'errorB'])
    print(df)
    df = df.replace(to_replace='None', value=np.nan).dropna()
    
    df = df.astype({"timestamp": int, "inputA": float, "outputA": float, "a_adjust": float, "errorA": float, "inputB": float, "outputB": float, "b_adjust": float, "errorB": float})
...
for filename in os.listdir(data_dir):
    ...
    if filename.endswith('.csv'):
        # if not os.path.exists(graphs_dir + f'/{filename[:-4]}.pdf'):
